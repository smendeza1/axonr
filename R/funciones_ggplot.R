...
theme_axon <- function(base_family = "Open Sans", base_size = 11) {
    ...
    theme_bw(base_size = base_size, base_family = base_family) +
      theme(panel.border = element_rect(color = color.background),
        panel.grid.major = element_line(color = color.grid.major, size = 0.25, linetype = 2),
        ...
        legend.text = element_text(size = rel(0.8), color = color.axis.title),
        plot.title = element_text(color = color.axis.title, size = rel(1.2), vjust = 1.25),
        axis.text.x = element_text(size = rel(0.9), color = color.axis.text),
        axis.text.y = element_text(size = rel(0.9), color = color.axis.text),
